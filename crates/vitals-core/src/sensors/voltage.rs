...
pub struct VoltageProvider {
    sensors: Vec<HwmonSensor>,
}

impl VoltageProvider {
    pub fn new() -> Self {
        Self {
            sensors: Vec::new(),
        }
...
impl SensorProvider for VoltageProvider {
    fn discover(&mut self) -> Vec<SensorReading> {
        let all = hwmon::discover_hwmon_sensors(false, true, false, false);
        self.sensors = all
            .into_iter()
            .filter(|s| s.category == SensorCategory::Voltage)
            .collect();
        self.query(0.0)
    }
